import ollama
import sys_msgs #file containing system messages 
import requests 
import trafilatura #text scraper library
from  bs4 import BeautifulSoup #python web fetcher library
from colorama import init, Fore, Style

init(autoreset=True)

assistant_convo=[sys_msgs.assistant_msg] #EMPTY LIST FOR STORING RESPONSES

#AGENT FUNCTION THAT RECEIVES USER INPUT AND DECIDES WHETHER TO SEARCH OR NOT
def search_or_not():
...
#WORKER FUNCTION TO SEARCH THE QUERY ON DUCKDUCKGO
def duckduckgo_search(query):
    ...
    for i, result in enumerate(soup.find_all('div',class_='result'),start=1):
        ...
        snippet=snippet_tag.text.strip() if snippet_tag else 'No descripton available'
...
#AGENT TO FETCH 10 BEST SEARCH RESULTS FROM DUCKDUCKGO
...
#WORKER FUNCTION TO SCRAPE THE BEST SEARCH RESULT AMONG THE 10 SEARCH RESULTS RECEIVED FROM DUCKDUCKGO
...
def contains_data_needed(search_content, query):
    ...
    needed_prompt=f'PAGE TEXT: {search_content} \nUSER_PROMPT: {assistant_convo[-1]} \nSEARCH_QUERY: {query}'
...
#AI AGENT TO HANDLE WEB SEARCH LOGIC, RETURNS CONTEXT FOR OUR PROMPT
def ai_search():
    context= None
...
#STREAM AGENT TO STORE DATA AS IT COMES
